...
namespace Watermelon
{
    ...
    public sealed partial class CinemachineCameraController : MonoBehaviour
    {
        ...
        public static void Expand(float time, SimpleCallback callback)
        {
            ...
            {
                var transposer = activeVirtualCamera.VirtualCamera.GetComponent<CinemachineFollow>();
                if (transposer != null)
                {
                    Vector3 startValue = transposer.FollowOffset;
                    Tween.DoFloat(0, 1, 0.8f, (result) =>
                    {
                        transposer.FollowOffset = Vector3.Lerp(startValue, startValue * 2, result);
